...
import { 
  ...
  MapPin, Landmark, Workflow, Code2,
...
const TABS: Tab[] = [
  ...
  { id: 'economics', label: 'PRICING', icon: ZapIcon },
...
const getAuraColors = () => {
    return [
      { color: 'cyan' as const, top: '-10%', left: '10%', size: 600, opacity: 0.36 },
      { color: 'violet' as const, top: '30%', right: '-5%', size: 500, opacity: 0.30 },
      { color: 'emerald' as const, bottom: '20%', left: '-10%', size: 400, opacity: 0.24 },
      { color: 'pink' as const, bottom: '-5%', right: '20%', size: 350, opacity: 0.18 },
    ];
};
...
export const FullPitch01Page: React.FC = () => {
  ...
  useEffect(() => {
    // Force GEEK mode for full effects
    setMode('GEEK');
  ...
  return (
    <main className="relative z-10 min-h-screen bg-gradient-to-br from-teal-950 via-slate-950 to-pink-950 text-white overflow-x-hidden font-sans selection:bg-cyan-500/30">
      ...
      {/* BACKGROUND ENGINE - WIRE AESTHETIC */}
      ...
      {/* JARVIS INTRO - FIXED CENTER BANNER */}
      <AnimatePresence>
        {showBanner && (
          <motion.div
          ...
          >
            <div className="relative group">
              ...
              <motion.div 
                ...
                className="absolute -inset-12 rounded-full border-2 border-dashed border-cyan-500/20"
              />
              <motion.div 
                animate={{ rotate: -360 }}
                transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                className="absolute -inset-16 rounded-full border border-dotted border-violet-500/10"
              />

              <div className="px-12 py-6 rounded-2xl border border-cyan-400/50 bg-black/90 backdrop-blur-3xl shadow-[0_0_50px_rgba(34,211,238,0.3)] flex flex-col items-center gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-3 h-3 rounded-full bg-cyan-400 animate-pulse" />
                  <div className="text-xs font-mono text-cyan-400 font-black tracking-[0.3em] uppercase">
                    Jarvis Protocol Active
                  </div>
                </div>
                <div className="text-3xl sm:text-4xl font-black text-white uppercase italic tracking-tighter whitespace-nowrap">
                  Welcome to the future, <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-violet-400">Investor</span>
                </div>
                <div className="h-0.5 w-full bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent mt-2" />
                <div className="text-[10px] font-mono text-white/40 uppercase tracking-widest font-bold">
      ...
      {/* NAVIGATION */}
      <nav className="sticky top-0 z-50 px-4 sm:px-6 py-6 bg-slate-950/80 backdrop-blur-xl border-b border-white/5 supports-[backdrop-filter]:bg-slate-950/60">
          ...
          <div className="flex flex-nowrap justify-start lg:justify-center gap-2 min-w-max pb-2">
            {TABS.map((tab) => (
              <button
                ...
                className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border flex items-center gap-2 group relative overflow-hidden ${
                  activeTab === tab.id
                    ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.2)]'
                    : 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white border-transparent'
                }`}
              >
                <div className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000 ${activeTab === tab.id ? 'animate-shimmer' : ''}`} />
                <tab.icon className={`w-3 h-3 ${activeTab === tab.id ? 'text-cyan-400' : 'text-white/40 group-hover:text-white'}`} />
                {tab.label}
              </button>
            ))}
          </div>
      ...
      {/* CONTENT ENGINE */}
      <div className="px-4 sm:px-6 py-20 min-h-[80vh]">
        <div className="max-w-7xl mx-auto">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'vision' && (
              ...
              >
                <div className="p-12 sm:p-20 rounded-[3rem] border border-cyan-500/20 bg-slate-950/50 backdrop-blur-md max-w-5xl mx-auto text-center shadow-[0_0_50px_rgba(34,211,238,0.1)] relative overflow-hidden">
                  <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(6,182,212,0.05)_50%,transparent_75%)] bg-[length:250%_250%] animate-shine pointer-events-none" />
                  ...
                  <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-10 leading-tight uppercase font-display italic">
                    ...
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-pink-500">Join the revolution.</span>
                  </h2>
                  <p className="text-slate-300 text-xl md:text-2xl leading-relaxed font-medium mb-16 max-w-3xl mx-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="p-8 rounded-3xl bg-slate-900/50 border border-white/5 group hover:border-cyan-500/30 transition-all shadow-2xl relative overflow-hidden">
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-2">Initial CAC</div>
                      </div>
                    </div>
                    <div className="p-8 rounded-3xl bg-slate-900/50 border border-white/5 group hover:border-pink-500/30 transition-all shadow-2xl relative overflow-hidden">
                       ...
                       <div className="relative z-10">
                        ...
                        <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-2">Activation Retention</div>
                      </div>
                    </div>
                    <div className="p-8 rounded-3xl bg-slate-900/50 border border-white/5 group hover:border-emerald-500/30 transition-all shadow-2xl relative overflow-hidden">
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="text-[10px] text-slate-400 uppercase font-black tracking-widest mt-2">Concept to MVP</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeTab === 'portfolio' && (
              ...
              >
                <div className="text-center">
                  <h2 className="text-6xl font-black uppercase italic tracking-tighter text-white leading-none">Portfolio Value<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-cyan-400">Multiplier Engine</span></h2>
                  <p className="text-slate-400 text-xl uppercase tracking-widest mt-6">15% Equity // 45%+ IRR Target // Systematic Value Capture</p>
                </div>
                
                <div className="p-8 rounded-[3rem] border border-amber-500/20 bg-amber-950/20 backdrop-blur-xl max-w-4xl mx-auto text-center relative overflow-hidden">
                  ...
                  <p className="text-slate-300 text-sm leading-relaxed uppercase font-bold tracking-tight px-12">
              ...
              >
                ...
                <div className="flex flex-col items-center gap-12 relative max-w-7xl mx-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
                    {[
                      { name: 'Strategic Advisor', role: 'Strategic Board', status: 'ACTIVE', color: 'violet', initials: 'SA', d: 'Fundraising & Global Expansion Strategy.' },
              ...
              >
                <div className="space-y-12">
                  <div className="text-center">
                    ...
                    <p className="text-slate-400 font-mono text-xs uppercase tracking-[0.5em] mt-4">1.4% RETENTION_BASE // JETBRAINS_MONO_DATA</p>
                  </div>
                  <AnalyticsTab />
                </div>
              </motion.div>
            ...
            {activeTab === 'market' && (
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-slate-400 font-mono text-xs uppercase tracking-[0.5em] mt-6 font-black">TAM $36.2B // STRATEGIC_INTERCEPT_V4</p>
                </div>
                      ...
                      {[
                        ...
                        <div key={idx} className="flex items-start gap-6 group">
                          ...
                          <div>
                            ...
                            <p className="text-slate-400 text-[11px] font-bold mt-4 uppercase leading-relaxed max-w-sm tracking-tight">{item.desc}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === 'gtm' && (
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-slate-400 font-mono text-xs uppercase tracking-[0.5em] mt-8 font-black">PHASED_GLOBAL_ROLLOUT // GTM_V6</p>
                </div>
                <div className="space-y-12">
                  {[
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="grid grid-cols-1 gap-4 pt-4">
                            {p.items.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-6"><div className={`w-1.5 h-1.5 rounded-full bg-${p.color}-500`} /><span className="text-sm font-black uppercase text-slate-400 tracking-widest">{item}</span></div>
                            ))}
                          </div>
                        </div>
            ...
            {activeTab === 'risk' && (
              ...
              >
                <div className="text-center">
                  ...
                  <p className="text-slate-400 font-mono text-xs uppercase tracking-[0.5em] mt-6">THREAT_VECTOR_ASSESSMENT // DEFENSIBILITY_IP</p>
                </div>
                <DynamicRiskMatrix />
                <RiskSection />
              </motion.div>
            ...
            {/* PRICING SECTION - MOVED INSIDE ANIMATEPRESENCE FOR SMOOTHNESS */}
            {activeTab === 'economics' && (
              <motion.div 
                key="economics" 
                initial={{ opacity: 0, scale: 0.95, filter: 'blur(10px)' }} 
              ...
              >
                ...
                <div className="space-y-32">
                  <div className="flex flex-col md:flex-row items-center justify-between gap-12 mb-20 bg-slate-900/50 p-10 rounded-[3rem] border border-white/10">
                    <div className="flex items-center gap-8">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-sm font-black uppercase tracking-[0.4em] font-mono italic mt-2">SYSTEM_PPP_ENABLED // RO_IN_US_UK_EU</p>
                      </div>
                    </div>
                    <div className="flex p-3 rounded-3xl bg-black/60 border border-white/10 backdrop-blur-3xl shadow-inner">
                      <button onClick={() => setPricingRegion('emerging')} className={`px-12 py-5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${pricingRegion === 'emerging' ? 'bg-cyan-500 text-black shadow-2xl shadow-cyan-500/40 scale-105' : 'text-slate-400 hover:text-white/60'}`}>Emerging Markets</button>
                      <button onClick={() => setPricingRegion('west')} className={`px-12 py-5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${pricingRegion === 'west' ? 'bg-violet-500 text-white shadow-2xl shadow-violet-500/40 scale-105' : 'text-slate-400 hover:text-white/60'}`}>West (US/UK/EU)</button>
                    </div>
                  </div>
                  ...
                  <div className="flex overflow-x-auto gap-8 pb-20 no-scrollbar snap-x snap-mandatory px-4">
                    {(pricingRegion === 'emerging' ? PRICING_EMERGING : PRICING_WEST).map((tier, idx) => (
                      ...
                      >
                        ...
                        <div className="text-xs font-black text-slate-400 uppercase mb-8 tracking-[0.4em] font-mono italic border-b border-white/5 pb-4">{tier.target}</div>
                        <div className="text-3xl font-black text-white uppercase mb-6 tracking-tighter italic font-display">{tier.name}</div>
                        <div className="text-7xl font-black text-white font-mono mb-16 tracking-tighter italic underline decoration-cyan-500/20 underline-offset-12 decoration-8">{tier.price}<span className="text-2xl text-slate-400 font-black">/mo</span></div>
                        <ul className="space-y-8 flex-1 mb-20">
                          {tier.features.map((f, i) => (
                            <li key={i} className="flex items-center gap-6 text-sm font-black uppercase text-slate-400 tracking-widest leading-none group-hover:text-white transition-colors">
      ...
      <footer className="relative border-t border-white/5 py-40 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-16 relative z-10">
          ...
          <p className="text-slate-400 text-2xl font-medium leading-relaxed max-w-3xl mx-auto italic uppercase tracking-tight">
          ...
          <div className="pt-20 text-slate-700 font-mono text-[10px] uppercase tracking-[0.6em] font-black">2026_APEX_OS_SOVEREIGNTY_PROTOCOL // ALL_RIGHTS_RESERVED // KERNEL_LOCK_ACTIVE</div>
        </div>
      </footer>
    </main>
  );
