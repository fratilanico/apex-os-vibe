...
export const SpectacularTerminal: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSuggestionRotationIndex(p => p + 1);
    }, 8000);
  ...
  const getDynamicSuggestions = useCallback((): string[] => {
    ...
    // Pool of suggestions categorized by intent/context
    const pool = {
      mvp: [
        'Give me 3 MVP definitions for my idea and pick the fastest one.',
        'Infer the likely product and propose the smallest measurable outcome in 10 days.',
        'Turn "build mvp" into a concrete scope: 3 user stories + success metrics.',
        'What is a "Skeleton MVP" and how do I build one this weekend?',
        'Show me the difference between a feature-rich MVP and a Lean MVP.'
      ],
      mindset: [
        ...
        'How do I stop thinking like a coder and start thinking like an architect?',
        'What does "Direct Neural Link" mean for founder productivity?'
      ],
      help: [
        'Show me all available terminal commands.',
        'How do I change my trajectory (BLUE vs RED pill)?',
        'What can JARVIS help me with?',
        'Explain the different terminal color modes.',
        'How do I access the private resource vault?'
      ],
      automation: [
        'What are the first 3 workflows to automate with AI?',
        'How do I build a content pipeline with AI agents?',
        'Show me a lead scoring system design for a B2B startup.',
        'How to automate customer support without losing the human touch?',
        'What tools do I need for a production-ready agent swarm?'
      ],
      strategy: [
        'Help me define my 12-month vision for AI integration.',
        'What is the ROI of an AI-first ops strategy?',
        'How to outpace competitors using 17-agent swarms?',
        'Design a recruitment strategy for an AI-native company.',
        'What are the common pitfalls of rapid AI adoption?'
      ],
      pricing: [
        'Calculate the ROI',
      ...
      discovery: [
        'How does Module 00: The Shift work?',
        'What is the 30-day plan to full automation?',
        'Explain the 17-agent swarm architecture.',
        'What is the "Direct Neural Link"?',
        'Show me the roadmap for APEX OS.'
      ],
      defaults: [
        ...TRAJECTORY_CONFIG[tId].defaultPrompts,
        'How does the 17-agent swarm work?',
        'Show me the roadmap for APEX OS.',
        'What is the "Direct Neural Link" handshake?',
        'Can I use APEX OS for mobile development?',
        'How do I invite my team to the swarm?'
      ]
    };

    let matches: string[] = [];

    // Contextual matching logic - Prioritize specific intent
    if (/\bmvp\b|\bbuild\b|\bproduct\b/.test(t)) matches = [...matches, ...pool.mvp];
    if (/module\s*00?|\bshift\b|\bmindset\b|\bfounder\b/.test(t)) matches = [...matches, ...pool.mindset];
    if (/\?|\bhelp\b|\bcommands\b|\bhow\b/.test(t)) matches = [...matches, ...pool.help];
    if (/\bauto\b|\bworkflow\b|\bagent\b|\bswarm\b/.test(t)) matches = [...matches, ...pool.automation];
    if (/\bvision\b|\bstrategy\b|\bplan\b|\bgrowth\b/.test(t)) matches = [...matches, ...pool.strategy];
    if (/\bprice\b|\bcost\b|\bpay\b|\bmoney\b|\broi\b|\btier\b/.test(t)) matches = [...matches, ...pool.pricing];
    if (/\btech\b|\bstack\b|\bai\b|\bmodel\b|\bsecurity\b|\bcode\b/.test(t)) matches = [...matches, ...pool.tech];

    // If we have contextual matches, show them.
    if (matches.length > 0) {
      const unique = Array.from(new Set(matches));
      // Using suggestionRotationIndex ensures prompts cycle even if context stays same
      const startIndex = suggestionRotationIndex % unique.length;
      return [...unique, ...unique].slice(startIndex, startIndex + 3);
    }

    // Idle / Default state: Rotate through high-value discovery prompts
    // We merge discovery + defaults to ensure variety
    const idlePool = [...pool.discovery, ...pool.defaults];
    const uniqueIdle = Array.from(new Set(idlePool));
    const start = suggestionRotationIndex % uniqueIdle.length;
    
    return [...uniqueIdle, ...uniqueIdle].slice(start, start + 3);
  }, [trajectory, lastUserMessage, suggestionRotationIndex]);
  ...
  // Recovery: if terminal is empty and step is persisted from old session, reset to boot
  useEffect(() => {
    // If we have no lines AND we're not in boot/idle, the user likely has stale persisted state
    // Force reset to show the boot sequence
    if (lines.length === 0 && step !== 'boot' && step !== 'idle' && !isUnlocked) {
      setStep('boot');
      setBootLine(0);
      logoRenderedRef.current = false;
    }
  }, [lines.length, step, isUnlocked, setStep]);
  ...
  const handleChat = async (msg: string) => {
    ...
    try {
      ...
      const context = `The user is on the "/waitlist" page. Mode: ${mode}. Sync Level: ${isUnlocked ? 'TIER 1 (UNLOCKED - HANDSHAKE COMPLETE)' : 'TIER 0'}. User persona: ${persona || 'undetermined'}. Trajectory: ${trajectory || 'unset'}. StrictMode: ${strictMode ? 'on' : 'off'}. ${isUnlocked ? 'CRITICAL: User has COMPLETED handshake. NEVER mention "handshake phase" or gate them. They have full access.' : 'Access granted to Module 00/01.'} System: Stark-V3. StateHints: ${stateHints}`;
      ...
      // Provider debug info hidden from users
      // addTerminalLine(
        ...
        <AnimatePresence>
          {lines.map((line) => (
            <motion.div
              ...
              className={line.type === 'ascii' ? '' : `text-xs sm:text-sm leading-relaxed whitespace-pre-wrap ${
        ...
        {showPillChoice && (
          ...
          >
            ...
            <div className="mt-4 text-center">
              <p className="text-[11px] text-white/40 font-light italic">
                Or start typing — change later with <span className="text-cyan-300/70">blue</span> or <span className="text-rose-300/70">red</span>
              </p>
            </div>
          </motion.div>
        )}
                  ...
                  {suggestionPrompts.map((p, index) => (
                    <motion.button
                      ...
                      animate={{ 
                        ...
                        y: 0,
                        boxShadow: [
                          '0 0 20px -5px rgba(236, 72, 153, 0.3), 0 0 40px -10px rgba(236, 72, 153, 0.15), inset 0 1px 1px rgba(255,255,255,0.1)',
                          '0 0 30px -5px rgba(236, 72, 153, 0.5), 0 0 60px -10px rgba(236, 72, 153, 0.25), inset 0 1px 1px rgba(255,255,255,0.15)',
                          '0 0 20px -5px rgba(236, 72, 153, 0.3), 0 0 40px -10px rgba(236, 72, 153, 0.15), inset 0 1px 1px rgba(255,255,255,0.1)'
                        ],
                        transition: {
                          type: "spring",
                      ...
                      whileHover={{ 
                        scale: 1.03,
                        boxShadow: '0 0 40px -5px rgba(236, 72, 153, 0.6), 0 0 80px -10px rgba(236, 72, 153, 0.35), inset 0 1px 1px rgba(255,255,255,0.2)',
                      }}
                      whileTap={{ scale: 0.97 }}
                      onClick={() => {
                        setInputValue(p);
                      ...
                      className="shrink-0 px-4 py-2 rounded-xl bg-gradient-to-br from-pink-500/20 via-rose-500/15 to-fuchsia-600/20 border border-pink-400/40 text-pink-100 text-[11px] font-medium tracking-wide backdrop-blur-sm transition-all"
                      style={{
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleCommand(inputValue); }}
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setLastUserMessage(e.target.value);
            }}
