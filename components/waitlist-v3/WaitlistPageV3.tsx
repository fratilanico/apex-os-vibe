...
const WaitlistPageV3: React.FC = () => {
  ...
  const getAuraColors = () => {
    if (persona === 'PERSONAL') return [
      { color: 'cyan', top: '-10%', left: '10%', size: 800, opacity: 0.55 },
      { color: 'cyan', top: '40%', right: '-5%', size: 700, opacity: 0.45 },
    ];
    if (persona === 'BUSINESS') return [
      { color: 'violet', top: '-10%', left: '10%', size: 800, opacity: 0.55 },
      { color: 'pink', top: '40%', right: '-5%', size: 700, opacity: 0.45 },
    ];
    return [
      { color: 'cyan', top: '-10%', left: '10%', size: 800, opacity: 0.55 },
      { color: 'violet', top: '30%', right: '-5%', size: 700, opacity: 0.45 },
    ];
  };

  return (
    <div className="relative min-h-screen w-full text-white overflow-x-hidden" style={{ background: 'linear-gradient(135deg, #0a1628 0%, #0d2137 25%, #0a2a3a 50%, #0d2137 75%, #0a1628 100%)' }}>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 pt-16 pb-12">
        ...
        <div className="py-16 text-center opacity-60 hover:opacity-100 transition-opacity" id="apply">
          {submitted && result ? <SuccessState {...result} /> : <ApplicationForm onSuccess={handleSuccess} />}
        </div>
        <WaitlistFooter />
      </main>
