...
export type ModuleTier = 'free' | 'core' | 'pro' | 'elite' | 'enterprise';
export type ModulePhase = 'foundation' | 'learner' | 'builder' | 'visionary' | 'accelerator';
...
export interface Module {
  ...
  icon: string;                  // Lucide icon name
  tier: ModuleTier;              // Access level required
  phase: ModulePhase;            // Journey stage
  level: number;                 // Level rank
  xpRequired: number;            // Total XP to unlock
  description?: string;          // Short grid description
}
