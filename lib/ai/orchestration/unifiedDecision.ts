...
function looksLikeRigidGoalGate(text: string): boolean {
  ...
  return (
    ...
    // Phase-based rigid gate language
    t.includes('handshake phase') ||
    t.includes('processing phase') ||
    t.includes('validation phase') ||
    t.includes('discovery phase') ||
    (t.includes('currently in') && t.includes('phase')) ||
    t.includes('mission parameters') ||
    t.includes('adhere to') ||
    t.includes('we must') ||
    t.includes('your query is noted')
  );
}
