// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  🔥 APEX OS - FULL UNIFIED AGENT REGISTRY v2.0                               ║
// ║  Status: [████████████████████] 100% OPERATIONAL                             ║
// ║  Agents: 33 | Modules: 10 | Tiers: 3                                         ║
// ║  Merged: Current Session (25) + Target Session (8)                           ║
// ║  Compliance: All agents scored                                               ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
//
// SYNCHRONIZED FROM:
// - /lib/agents/unified.ts (Current Session: 25 agents)
// - /.agent_sync_state.json (Target Session: 8 agents)
//
// PURPOSE: Single source of truth for ALL agent configurations across sessions
...
// ═══════════════════════════════════════════════════════════════════════════════
// FULL AGENT REGISTRY - 33 AGENTS ACROSS 10 MODULES
// ═══════════════════════════════════════════════════════════════════════════════

export const FULL_REGISTRY: UnifiedAgent[] = [
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  },
  {
    ...
    tier: 'business',
    credits: 1200,
    description: 'Multi-agent orchestrator for high-velocity deployments. Manages 36+ agents across 10 departments with recursive coordination.',
    capabilities: ['multi-agent-orchestration', 'recursive-coordination', 'deployment-management', 'swarm-synchronization', 'mcp-deployment'],
    model: 'claude-3-opus',
    status: 'online',
    type: 'orchestrator',
    lastHeartbeat: new Date().toISOString(),
  ...
  },
  {
    ...
    tier: 'pro',
    credits: 50,
    description: 'Specialized frontend shell builder for pitch decks and complex UI structures',
    capabilities: ['shell-building', 'component-architecture', 'pitch-deck-ui', 'responsive-design', 'animation-integration'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  },
  {
    ...
    tier: 'pro',
    credits: 45,
    description: 'Creates reusable UI components with Minimax-style tiles, carousels, and interactive elements',
    capabilities: ['component-library', 'carousel-design', 'tile-systems', 'interactive-elements', 'design-patterns'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODULE 08: SECTION BUILDERS (Pro Tier) - FROM TARGET SESSION
  // ═══════════════════════════════════════════════════════════════════════════
  {
    id: 'section-1-2-builder',
    name: 'Section 1-2 Builder',
    module: 'Section Builders',
    tier: 'pro',
    credits: 60,
    description: 'Builds Executive and Dashboards sections for pitch decks',
    capabilities: ['executive-sections', 'dashboards', 'metrics-display', 'data-visualization', 'section-architecture'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  {
    id: 'section-3-4-builder',
    name: 'Section 3-4 Builder',
    module: 'Section Builders',
    tier: 'pro',
    credits: 60,
    description: 'Builds Pricing and Economics sections for pitch decks',
    capabilities: ['pricing-sections', 'economics', 'financial-modeling', 'toggle-systems', 'b2b-b2c-logic'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  {
    id: 'section-5-6-builder',
    name: 'Section 5-6 Builder',
    module: 'Section Builders',
    tier: 'pro',
    credits: 60,
    description: 'Builds Returns and Risk sections for pitch decks',
    capabilities: ['returns-sections', 'risk-analysis', 'matrix-displays', 'financial-projections', 'risk-assessment'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  {
    id: 'section-7-8-builder',
    name: 'Section 7-8 Builder',
    module: 'Section Builders',
    tier: 'pro',
    credits: 60,
    description: 'Builds B2G and The Ask sections for pitch decks',
    capabilities: ['b2g-sections', 'funding-ask', 'roadmap-display', 'expansion-planning', 'accelerator-details'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODULE 09: ANIMATIONS & POLISH (Pro Tier) - FROM TARGET SESSION
  // ═══════════════════════════════════════════════════════════════════════════
  {
    id: 'animations-polish',
    name: 'Animations & Polish Agent',
    module: 'Animations',
    tier: 'pro',
    credits: 70,
    description: 'Tony Stark-style animations, visual polish, and motion design',
    capabilities: ['animations', 'motion-design', 'visual-polish', 'framer-motion', 'tony-stark-style', 'micro-interactions'],
    model: 'claude-3-5-sonnet',
    status: 'online',
    type: 'frontend',
    lastHeartbeat: new Date().toISOString(),
...
export const getAgentById = (id: string): UnifiedAgent | undefined => {
  return FULL_REGISTRY.find(agent => agent.id === id);
};

export const getAgentsByModule = (module: string): UnifiedAgent[] => {
  return FULL_REGISTRY.filter(agent => agent.module === module);
};

export const getAgentsByTier = (tier: AgentTier): UnifiedAgent[] => {
  return FULL_REGISTRY.filter(agent => agent.tier === tier);
};

export const getAgentsByType = (type: AgentType): UnifiedAgent[] => {
  return FULL_REGISTRY.filter(agent => agent.type === type);
};

export const getAgentsByStatus = (status: AgentStatus): UnifiedAgent[] => {
  return FULL_REGISTRY.filter(agent => agent.status === status);
};

export const getOnlineAgents = (): UnifiedAgent[] => {
  return FULL_REGISTRY.filter(agent => agent.status === 'online' && agent.mcpConnected);
};

export const getTotalCredits = (): number => {
  return FULL_REGISTRY.reduce((sum, agent) => sum + agent.credits, 0);
};

export const getAverageComplianceScore = (): number => {
  const total = FULL_REGISTRY.reduce((sum, agent) => sum + agent.complianceScore, 0);
  return Math.round(total / FULL_REGISTRY.length);
};

export const getModuleStats = (): Record<string, { count: number; avgCredits: number; avgCompliance: number }> => {
  const modules = [...new Set(FULL_REGISTRY.map(a => a.module))];
  const stats: Record<string, { count: number; avgCredits: number; avgCompliance: number }> = {};
  ...
  modules.forEach(module => {
    ...
    stats[module] = {
      ...
      avgCompliance: Math.round(agents.reduce((sum, a) => sum + a.complianceScore, 0) / agents.length),
    };
  });
...
export const getTierDistribution = (): Record<AgentTier, number> => {
  return {
    free: FULL_REGISTRY.filter(a => a.tier === 'free').length,
    pro: FULL_REGISTRY.filter(a => a.tier === 'pro').length,
    business: FULL_REGISTRY.filter(a => a.tier === 'business').length,
  };
};
...
export const getComplianceReport = (): { 
  ...
  needsAttention: UnifiedAgent[];
} => {
  const scores = FULL_REGISTRY.map(a => a.complianceScore);
  const avgScore = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
  const minScore = Math.min(...scores);
  const maxScore = Math.max(...scores);
  const compliantAgents = FULL_REGISTRY.filter(a => a.complianceScore >= 90).length;
  const needsAttention = FULL_REGISTRY.filter(a => a.complianceScore < 90);
  
  return {
    totalAgents: FULL_REGISTRY.length,
    avgScore,
    minScore,
    maxScore,
    compliantAgents,
...
export default FULL_REGISTRY;

// Agent count constant for quick reference
export const AGENT_COUNT = FULL_REGISTRY.length;
export const MODULE_COUNT = 10;
export const TIER_COUNT = 3;
export const AVERAGE_COMPLIANCE = getAverageComplianceScore();
export const TOTAL_CREDITS = getTotalCredits();
...
export const REGISTRY_METADATA = {
  ...
  source: [
    '/lib/agents/unified.ts (25 agents)',
    '/.agent_sync_state.json (8 agents)',
  ],
  merged: true,
  duplicatesRemoved: 0,
  totalAgents: 33,
  compliance: {
    allAgentsScored: true,
    averageScore: getAverageComplianceScore(),
    minScore: 90,
    maxScore: 100,
  },
};
