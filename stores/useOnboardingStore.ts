...
interface OnboardingState {
  ...
  lastUpdated: number; // Timestamp for 14-day persistence check
...
export const useOnboardingStore = create<OnboardingState>()(
  persist(
    (set, get) => ({
      ...
      setPersona: (persona) => {
        set({ persona, lastUpdated: Date.now() });
        const userId = getOrCreateUserId();
        const { currentSessionId } = get();
        trackUserInteraction({
          user_id: userId,
      ...
      setEmail: (email) => set({ email, lastUpdated: Date.now() }),
      setGoal: (goal) => set({ goal, lastUpdated: Date.now() }),
      addHistory: (line) => set((state) => ({ history: [...state.history, line].slice(-100), lastUpdated: Date.now() })),
      toggleTerminalOnly: () => set((state) => ({ isTerminalOnly: !state.isTerminalOnly, lastUpdated: Date.now() })),
      unlock: () => {
        set({ isUnlocked: true, lastUpdated: Date.now() });
        const userId = getOrCreateUserId();
        const { currentSessionId } = get();
        trackUserInteraction({
          user_id: userId,
      ...
      setSecretTreatFound: (found) => set({ secretTreatFound: found, lastUpdated: Date.now() }),
      setVaultOpen: (open) => {
        set({ isVaultOpen: open, lastUpdated: Date.now() });
        const userId = getOrCreateUserId();
        const { currentSessionId } = get();
        trackUserInteraction({
          user_id: userId,
      ...
      setVoiceEnabled: (enabled) => set({ voiceEnabled: enabled, lastUpdated: Date.now() }),
      ...
      addJarvisMessage: (role, content) => {
        ...
        trackJarvisMessage({
          ...
          topics: [],
          sentiment: null,
          session_start_time: currentSession?.startTime || new Date().toISOString(),
          session_duration_seconds: currentSession 
            ? Math.floor((Date.now() - new Date(currentSession.startTime).getTime()) / 1000)
            : 0,
      ...
      reset: () => {
        localStorage.removeItem('apex-onboarding-storage');
        set({
          mode: 'STANDARD',
          step: 'boot',
          persona: null,
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          if (Date.now() - state.lastUpdated > FOURTEEN_DAYS) {
            state.reset();
          }
        }
      },
