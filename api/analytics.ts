...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Handle client trackEvent format (email, event_type, payload, etc.)
  if (req.body?.event_type) {
    const { email, event_type, payload, client_session_id, url } = req.body || {};
    
    const { error } = await supabaseServer
      .from('user_events')
      .insert({
        user_id: email || client_session_id || 'anonymous',
