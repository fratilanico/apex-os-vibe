...
const checkPerplexityHealth = async (): Promise<boolean> => {
  const apiKey = envTrim('PERPLEXITY_API_KEY');
  return !!apiKey; // Key exists = healthy. Let the actual call fail if there's a real issue.
};

function ensureADC(): void {
  // Strategy 1: Base64-encoded service account (Vercel deployments)
  const b64 = process.env.GOOGLE_APPLICATION_CREDENTIALS_BASE64;
  if (b64) {
    const targetPath = '/tmp/gcp-sa.json';
    try {
      // Always overwrite to pick up fresh credentials on redeploy
      fs.writeFileSync(targetPath, Buffer.from(b64, 'base64'));
      process.env.GOOGLE_APPLICATION_CREDENTIALS = targetPath;
      return;
    } catch (err) {}
  }
  // Strategy 2: Local ADC (dev machine with gcloud auth application-default login)
  // GoogleAuth will automatically find ~/.config/gcloud/application_default_credentials.json
  // or GOOGLE_APPLICATION_CREDENTIALS env var — no action needed
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const providers: AIProvider[] = [
      ...
      { name: 'vertex-fast', enabled: process.env.USE_VERTEX_AI !== 'false' && !!(process.env.GOOGLE_APPLICATION_CREDENTIALS_BASE64 || process.env.GOOGLE_APPLICATION_CREDENTIALS), priority: 2, call: callVertexFast },
      { name: 'vertex-pro', enabled: process.env.USE_VERTEX_AI !== 'false' && !!(process.env.GOOGLE_APPLICATION_CREDENTIALS_BASE64 || process.env.GOOGLE_APPLICATION_CREDENTIALS), priority: 3, call: callVertexPro },
    ];
